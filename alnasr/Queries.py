        ...
        for component in itemsFromFees.components:
            ...
            if component.balance > 0 and stopWhenNegative == True:
                ...
                if itemsFromPE.paid_amount > component.balance:
                    print(
                        f"this is the paid amount in positive {itemsFromPE.paid_amount}"
                    )
                    print("in positive")
                    ...
                    frappe.db.set_value(
                        "Fee Component", component.name, "balance", updatedBalance
                    )
                    component.balance = updatedBalance
                    try:
                ...
                if itemsFromPE.paid_amount < component.balance:
                    print(
                        f"this is the paid amount in negative {itemsFromPE.paid_amount}"
                    )
                    subtractionResult = (
                        itemsFromPE.paid_amount - component.balance
                    )  ##-100
...
def get_fee_components(fee_structure):
	...
	if fee_structure:
		fs = frappe.get_all(
			"Fee Component",
			fields=["fees_category", "description", "amount","fee_account"],
